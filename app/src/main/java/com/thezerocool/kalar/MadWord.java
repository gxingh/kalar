 ...
 */
public class MadWord extends Fragment implements View.OnClickListener {
    //For Scores:
...
//        scores = getActivity().getPreferences(Context.MODE_PRIVATE);
    ...
    public void gameOver() {
        handler.post(new Runnable() {
            ...
            public void run() {
                try {
                    int temp = scores.getInt(MAD_WORD_HIGH, 0);
                    highScoreValue = temp;
                    if (scoreValue >= temp) {
                        SharedPreferences.Editor editor = scores.edit();
                        editor.putInt(MAD_WORD_HIGH, MadWord.scoreValue);
